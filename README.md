...
var halley = new Planet(// wir könnten auch eine Komet Klasse erstellen, aber hier verwenden wir Planet für Einfachheit
    name: "Halley’scher Komet",
    katalogNummer: 40001,
    umlaufzeit: 76.0f,  // Umlaufzeit: ~76 Jahre
    katalogNummerReferenz: 10001  // Umkreist die Sonne (KatalogNummer 10001)
);
...
- Methode:
   ...
   - Berechne die Größe des Objekts mit `Marshal.SizeOf()`. (`using System.Runtime.InteropServices;`)
   ...
   - Link [Marshal](https://learn.microsoft.com/en-us/dotnet/api/System.Runtime.InteropServices.Marshal?view=net-10.0)
    
     - `Marshal.SizeOf(object)` gibt die Größe eines Objekts im Speicher zurück.
     - `Marshal.AllocHGlobal(int size)` reserviert unverwalteten Speicher.
     - `Marshal.Copy(IntPtr source, byte[] destination, int startIndex, int length)` kopiert Daten zwischen unmanaged memory und managed arrays.
     - `Marshal.StructureToPtr(object structure, IntPtr ptr, bool fDeleteOld)` kopiert eine Struktur in einen unverwalteten Speicherbereich.
     - `Marshal.FreeHGlobal(IntPtr hglobal)` gibt den reservierten unverwalteten Speicher frei.
...
- $r = a \cdot (1 - \epsilon^2) / (1 + \epsilon \cdot cos(θ))$
- Dabei gilt:

   - $a$: Große Halbachse
   - $\epsilon$: Exzentrizität
   - $\theta$: Winkel ($0$ bis $2\pi$)

![Ellipse from Wikimedia](https://upload.wikimedia.org/wikipedia/commons/9/9a/01-Ellipse-vertikal.svg?utm_source=commons.wikimedia.org&utm_campaign=index&utm_content=original "Petrus3743, CC BY-SA 4.0 <https://creativecommons.org/licenses/by-sa/4.0>, via Wikimedia Commons")

- $M$ Mittelpunkt
- $F_1$ und $F_2$ Brennpunkte
- $S_1, \dots, S_4$ Scheitelpunkte auf der Ellipse
- Hauptachse (rot), Nebenachse (grün)

2. Skalierung für die Konsole
=====================

- Skaliere die Koordinaten auf die Konsolengröße (z. B. 60x30 Zeichen).
- Zeilen repräsentieren die $y$-Koordinaten, Spalten die $x$-Koordinaten.
- Berechne die Skalierungsfaktoren für $x$ und $y$ basierend auf der großen Halbachse und der maximalen Höhe der Ellipse.
- Die Sonne (Brennpunkt) befindet sich bei $x = -a \cdot \epsilon$, $y$ = $0$ relativ zum Zentrum der Ellipse.
- Zeichne die Bahn, indem du für verschiedene Werte von $\theta$ die entsprechenden $x$- und $y$-Koordinaten berechnest und in einem 2D-Char-Array speicherst.
...
- Exzentrizität: Bei $\epsilon$ = $0$ ist die Bahn kreisförmig. Je größer $\epsilon$, desto elliptischer die Bahn. 
- Skalierung: Passe skalaX und skalaY an, um die Bahn in die Konsole zu passen.
- Winkel-Schrittweite: $\theta$ += $0.05$ für eine glatte Bahn, aber ASCII Art output ist auch schön mit größeren Schritten (z. B. $0.1$ oder $0.2$).
